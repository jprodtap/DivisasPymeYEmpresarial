...
import pages.AutogestionServicios.PageAutogestionServicioMiddle1;
import pages.actios.Pyme.PageLoginPymes1;

public class PageInformes1 extends BasePageWeb {
	/**
	...
	public PageInformes1(BasePageWeb parentPage) throws Exception {
		super(parentPage);
		this.pageLogin = new PageLoginPymes1(this);
	...
	By listTipoAutenticacion = By.id("cphCuerpo_ddlTipoAutenticacion"); // funciona para pagos masivos, informe de pagos
																		// y transferencias, informe usuarios creados
	...
	By listTipoIdentifEmp = By.id("cphCuerpo_ddlTipoIdentificacionEmpresa"); // funciona para pagos masivos, usuarios
																				// creados, transacciones FIC
	By inputNumIdentifEmp = By.id("cphCuerpo_txtNumIdentificacionEmpresa"); // funciona para pagos masivos, usuarios
																			// creados, autogestión, retiros sin tarjeta
	By inputNombreEmpCobros = By.id("cphCuerpo_txtNombreEmpresa"); // funciona para pagos masivos, servicios
																	// contratados, retiros sin tarjeta
	...
	By listTipoIdentificacionEmp = By.id("cphCuerpo_dropTipoIdentificacionEmpresa"); // funciona para informe
																						// autogestion
	...
	public boolean accesoAlSistema() throws Exception {
		...
		if (this.element(msjAlerta) != null) {
			...
			Evidence.save("Mensaje de alerta", this);
	...
	public boolean actividadesAdministradores() throws Exception {
		...
		Evidence.save("Párametros de búsqueda", this);
...
// se descarga archivo con resultados obtenidos
	...
	public boolean informeOperativo() throws Exception {
		...
		do {
			...
			if (this.element(tablaResultadosOperativo) != null
					&& this.element(tablaResultadosOperativo).getText().contains(FechaHoraNovedad)) {
				...
				for (WebElement fila : filas) {
					if (fila.getText().contains(FechaHoraNovedad)) {
						...
						btnVerDetalle.add(fila.findElement(
								By.xpath("//td[contains(text(),'" + FechaHoraNovedad + "')]/following-sibling::td")));
					}
...
//						System.out.println("("+filasTablaResultadosString+"//td[contains(text(),'" + FechaHoraNovedad + "')]/following-sibling::td)[" + i + "]");
						WebElement verDetalle = this.element("(" + filasTablaResultadosString + "//td[contains(text(),'"
								+ FechaHoraNovedad + "')]/following-sibling::td)[" + (i + 1) + "]");
						verDetalle.click();
						cont = 0;
						do {
							cont++;
						...
						if (guardarArchivo.contentEquals("Imprimir")) {
							...
							do {
								cont++;
								DXCUtil.wait(1);
							} while (this.element(tablaResultadosOperativo) == null && this.element(msjAlerta) == null
									&& cont < 8);
		...
		if (resultadoPrueba == false) {
			...
			mensajes = "Se encontró resultado de informe operativo con fecha " + "[" + FechaHoraNovedad
					+ "] de manera exitosa.";
		}
		...
		if (guardarArchivo.contentEquals("Imprimir")) {
			imprimirArchivos(btnImprimirListaOpcion2, btnImprimirImpresora, btnSalir, "Informe operativo");
			cont = 0;
			do {
				cont++;
				DXCUtil.wait(1);
			...
			if (this.element(btnGuardar) != null) {
				this.click(btnGuardar);
				DXCUtil.wait(6);
				Reporter.reportEvent(Reporter.MIC_INFO, "Se descarga el archivo de informe operativo correctamente.");
			} else {
				Reporter.reportEvent(Reporter.MIC_WARNING,
						"No permitio descargar archivo de informe operativo, no se encontró el botón guardar.");
			}
	...
	public boolean pagosMasivos() throws Exception {
		...
		if (numeroProceso != "") {
			...
			// Se da clic en el texto que coincida con el numero de proceso y se descarga la
			...
			} while (numProceso == null);

			if (resultadoPrueba == false) {
				mensajes = "No se encuentró ningún registro que coincida con el numero de proceso " + "["
						+ numeroProceso + "]";
			}

		} else if (numeroProceso == "") {
			Reporter.reportEvent(Reporter.MIC_INFO,
					"No se realiza búsqueda de número de proceso en informe dado que el campo no se diligencio en archivo de lanzamiento.");
		}
	...
	public boolean pagosYTransferencias() throws Exception {
		...
		if (FechaHoraNovedad != "") {
			...
			} while (buscaPago == null);

		} else {
			Reporter.reportEvent(Reporter.MIC_INFO,
					"No se realiza búsqueda de pago o transferencia por fecha y hora en informe dado que el campo no se diligencio en archivo de lanzamiento.");
		}
		// se descarga archivo con resultados obtenidos
		if (guardarArchivo.contentEquals("Imprimir")) {
	...
	public boolean serviciosContratados() throws Exception {
		...
		Reporter.reportEvent(Reporter.MIC_INFO,
				"Se ingresa al informe Servicios Contratados, se procede a realizar filtro de búsqueda");
		...
		if (this.element(btnSiguiente) != null) {
			...
			// se agrega if para tomar pantallas de la tabla cuando se tenga el botón
			// siguiente desahabilitado
			if (verificarClickSiguiente == true && this.element(btnSiguiente) == null) {
				Evidence.saveFullPage("Tabla Resultados", this);
				this.focus(tablaResultDescSobreRangoDestinoTarjPrep);
				DXCUtil.waitMilisegundos(500);
	...
	public boolean transaccionesACH() throws Exception {
		...
		if (guardarArchivo.contentEquals("Imprimir")) {
			imprimirArchivos(btnImprimirLista, btnImprimirImpresora, btnSalir,
					"Informe transacciones sin respuesta Ach");
	...
	public boolean transaccionesRealizadas() throws Exception {
		...
		if (guardarArchivo.contentEquals("Imprimir")) {
			imprimirArchivos(btnImprimirListaOpcion2, btnImprimirImpresora, btnSalir,
					"Informe transacciones realizadas");
	...
	public void imprimirArchivos(By btnImprimirTablaResultados, By btnImprimirSimboloImpresora,
		...
		WebElement imprimirTabla = this.element(btnImprimirTablaResultados);
		imprimirTabla.click();
		DXCUtil.wait(3);
...
//		Evidence.save("Evidencias imprimir archivo");
		this.element(btnImprimirSimboloImpresora).click();
		...
		PageAutogestionServicioMiddle1.toPrint(nombreArchivo);
	...
	public String selectDatePicker2(String element, String item) {
		...
		// de acuerdo a la fecha que llega toma el mes y convierte n�mero de
		for (int i = 0; i < months.length; i++) {
			if (Integer.parseInt(date[1]) == i + 1) {
				mesyAnio = months[i].toLowerCase() + ", " + date[2];
				break;
			}
		}
		...
		// Se busca el anio y mes para que seleccione el d�a
		while (!encontrado) {
			DXCUtil.wait(1);
			fechaDatePicker = this.element(element + "//div[@class='ajax__calendar_title']").getText();
			if (fechaDatePicker.equalsIgnoreCase(mesyAnio))
				encontrado = true;
			else
				this.element(element + "//div[@class='ajax__calendar_prev']").click();
		}

		this.element(element + "//td[not (contains(@class,'ajax__calendar_other'))]/div[text()='" + date[0] + "']").click();

		return "";
	}
	
	
	public String selectDatePickerFiducia(String item) {
		...
		while (!encontrado) {
			DXCUtil.wait(1);
			fechaDatePicker = this.element("//*[@class ='datepicker-days']//tr/th[@class='datepicker-switch']")
					.getText();
			if (fechaDatePicker.equalsIgnoreCase(mesyAnio))
				encontrado = true;
			else
				this.element("//*[@class ='datepicker-days']//tr/th[@class='datepicker-switch']/parent::tr/th[1]")
						.click();
		}

		this.element("//*[@class ='datepicker-days']/table/tbody/tr/td[text()='" + date[0]
				+ "'][@class='day' or @class='active day' ]").click();

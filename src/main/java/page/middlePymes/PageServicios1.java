...
public class PageServicios1 extends PageInicioMiddle1 {
	...
	public PageServicios1(PageLoginPymes1 parentPage) {
		super(parentPage);
	}
...
//***********************************************************************************************************************
	...
	public String parametrizarServicio(String servicio) throws Exception {
		...
		if (servicio.contentEquals("Libranza")) {
			...
			Evidence.save("Contratación Libranza",this);
		...
		if (servicio.equals("Recaudo")) {
			...
			Evidence.save("Recaudo Parametrizado",this);
			if (BaseD.contentEquals("SI")) {
				...
				if (this.element(CodigoEmp) != null) {
					...
					if (msgConvenio == null) {
						Evidence.save("Convenio Agregado",this);
						return msgConvenio;
					}
				}
...
// ***********************************************************************************************************************
	 ...
	 * @throws Exception
	 */
	public String seleOptionIgual(By locator, String opcion) throws Exception {
		String msg = "";
		...
		for (WebElement seleoption : options) {
			if (seleoption.getText().equals(opcion)) {
//                Devuelve la opcion selecionada
				String text = String.valueOf(seleoption.getAttribute("text"));

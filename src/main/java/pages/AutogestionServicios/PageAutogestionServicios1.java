package pages.AutogestionServicios;
...
public class PageAutogestionServicios1 extends BasePageWeb {

	public PageAutogestionServicios1(PageLoginPymes1 parentPage) {
		super(parentPage);
	}
...
// =======================================================================================================================
	...
	PagePortalPymes1 pagePortalPymes = null;
	...
	public List<HashMap<String, String>> contratarEliminarServicios() throws Exception {
		...
		do {
			...
			WebElement isTooltip = this.element(By.xpath(xpathShowTooltip));
			WebElement isTextTooltip = this.element(By.xpath(xpathTextTooltip));
			...
			if (this.tipoPrueba.equals("Eliminar")) {
				try {
					...
					WebElement isHireService = this.element(xpathDeleteService);
					boolean inInputDeleteDisabled = this.element(By.xpath(xpathInputDelete))
					...
					if (isHireService.isEnabled() && !inInputDeleteDisabled) {
						...
						if (isTooltip.isDisplayed() && isTextTooltip.isDisplayed()) {
							...
							Evidence.save("Servicios seleccionados",this);
				...
				try {
					...
					WebElement isServiceWoHire = this.element(xpathHireService);
					boolean inInputHireDisabled = this.element(By.xpath(xpathInputHire))
							.getAttribute("disabled") != null;
			...
			if (autorizar.equals("Si")) {
				...
				for (HashMap<String, String> contratado : data) {
					...
					if (tipoPrueba != null && tipoPrueba.equals("Contratar")) {
						...
						try {
							servicioLista = this.element(xpathServicios);
							DXCUtil.wait(2);
							checkboxServicio = this.element(xpathSelectServicio);
						} catch (Exception e) {
						...
						try {
							...
							if (servicioLista != null && checkboxServicio.isDisplayed() && checkboxServicio.isEnabled()
								...
								Evidence.save("Evidencia selección de servicio",this);
								DXCUtil.wait(2);
								if (servicioAprobado.equals("Apertura de Productos Digitales")
									...
									try {
										...
										try {
											elementoLimiteTx = this.element(By.xpath(
									...
									if (elementoLimiteTx != null) {
										...
										DXCUtil.wait(2);
										Evidence.save("Evidencia selección de limite",this);
										try {
											elementoValorLimiteTx = this.element(By.xpath(
													"//*[@id='cphCuerpo_gvServicio']/tbody/tr/td[contains(text(),'"
															+ servicioAprobado
															+ "')]/../td/table//td/table//label[contains(text(),'"
															+ limiteTx
										...
										if (elementoValorLimiteTx != null && limiteTx.equals("Limitado")) {
											...
											Evidence.save("Evidencia ingreso limite",this);
											...
											Evidence.save("Evidencia no se ingresa valor limite",this);
										}
									}
								}
							} else {
								...
								WebElement seleccEmpresa = this.element(selectEmpresa);
	...
	public void consultarAdminUsuarios() throws Exception {
		...
		Evidence.save("Evidencia Administracion de usuarios",this);
		DXCUtil.wait(2);
		List<WebElement> tableusers = this.findElements(tablaAdmiUsuarios);
		for (WebElement elemento : tableusers) {
			WebElement usuario = this.element(By.xpath(usuarios));
			if (usuario.isDisplayed()) {
				this.element(By.xpath(btnEditar)).click();
				DXCUtil.wait(5);
				Evidence.saveFullPage("Evidencia Administracion de usuarios", this);
			} else {
				WebElement btnSiguiente = this.element(btnSiguienteRg);
				btnSiguiente.click();
				Evidence.save("Evidencia Administracion de usuarios",this);
			}
		}
	...
	public void tarifas() throws Exception {
		...
		WebElement textTarifas = this
				.element(By.xpath("//*[@id='cphCuerpo_pnlContenido']/table[2]/tbody/tr/td/span[2]/p/b"));
		String tarifas = textTarifas.getText();
		WebElement linkAqui = this
				.element(By.xpath("//*[@id='cphCuerpo_pnlContenido']/table[2]/tbody/tr/td/span[2]/p/b/a"));
	...
	public String aprobarTransaccion() throws Exception {
		...
		this.pagePortalPymes = new PagePortalPymes1(this);

...
public class PageAutogestionServicioMiddle1 extends PageInicioMiddle1 {
	// Permite recolectar la información de los servicios que aprobo tx
	// List<HashMap<String, String>> results = new ArrayList<>();
	// String txDate;

	PageAutogestionServicios1 pageAutogestionServicios = null;
	...
	public PageAutogestionServicioMiddle1(PageLoginPymes1 parentPage) {
		super(parentPage);
		this.pageAutogestionServicios = new PageAutogestionServicios1(parentPage);
	}
	...
	private void adminEmpresas(int contarContratar, int contarEliminar) throws Exception {
		...
		Evidence.save("Ingresar datos de consulta",this);
		if (this.element(btnBuscEmpresa) != null) {
			this.click(this.element(btnBuscEmpresa));
		}
		...
		for (WebElement elemento : tableResults) {
			WebElement usuario = this.element(By.xpath(resultado));
		...
		DXCUtil.wait(3);
		WebElement txAcumulada = this.element(cantidadTxAcumuladasEmp);
	...
	private void procesarTipo(String tipoNovedad, String dateTx) throws Exception {
		...
		for (String xpathDetalle : xpathDetalles) {
			...
			this.element(By.xpath(xpathDetalle)).click();
			DXCUtil.wait(2);
			Evidence.saveFullPage("Registro de novedad", this);
			...
			DXCUtil.wait(2);
			this.element(btnRegresar).click();
	...
	public void imprimirResultados() throws Exception {
		...
		DXCUtil.wait(2);
		this.element(btnBuscarProcesos).click();
		Evidence.saveFullPage("Evidencias imprimir archivo", this);

		if (guardarArchivo.equals("Imprimir")) {
			DXCUtil.wait(3);
			WebElement imprimirLista = this.element(btnImprimirLista);
			imprimirLista.click();
			DXCUtil.wait(2);
			...
			this.element(btnImprimir).click();
			Evidence.saveFullPage("Evidencias imprimir archivo", this);
...
//		Evidence.save("Ingreso de datos",driver);
	}

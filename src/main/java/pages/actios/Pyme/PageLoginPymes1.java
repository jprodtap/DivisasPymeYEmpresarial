...
public class PageLoginPymes1 extends BasePageWeb {
...
//=======================================================================================================================
	public PageLoginPymes1(String navegador) {
		super(navegador);
		try {
			this.cargarUrls();
		} catch (Exception e) {
...
//***********************************************************************************************************************
	public PageLoginPymes1(String navegador, String downloadFilePath) {
...
//***********************************************************************************************************************
	public PageLoginPymes1(BasePageWeb parentPage) {
		super(parentPage);
	}
...
//***********************************************************************************************************************
	...
	private void cargarUrls() throws Exception {
		...
		if (PageLoginPymes1.URL_FRONT.equals("") || PageLoginPymes1.URL_MIDDLE.equals("")) {
//			DS_ProjectUrls datosUrl = new DS_ProjectUrls();
			
			ProjectUrls datosUrl = new ProjectUrls();
			PageLoginPymes1.URL_FRONT = datosUrl.getSiteUrl(DatosEmpresarial.AMBIENTE_TEST, "PYMES FRONT");
			PageLoginPymes1.URL_MIDDLE = datosUrl.getSiteUrl(DatosEmpresarial.AMBIENTE_TEST, "PYMES MIDDLE");
...
//				Evidence.save("HizoLogueo", this);
				Evidence.save("HizoLogueo",this);
			Evidence.save("HizoLogueoh",this);
		}
		return msgError;
	}
//***********************************************************************************************************************
...
//***********************************************************************************************************************
	...
	public static String seleccion() {
		...
		int posAmbiente;
		do {
			posAmbiente = JOptionPane.showOptionDialog(null, "Deseas volver a realizar la contrataciÃ³n ", null,
...
// -----------------------------------------------------------------------------------------------------------------------
		...
		String urlCargar = PageLoginPymes1.URL_FRONT;
		if (this.portalCargado.equals(DatosEmpresarial.PORTAL_MIDDLE_PYME))
			urlCargar = PageLoginPymes1.URL_MIDDLE;
// -----------------------------------------------------------------------------------------------------------------------
		// EMPIEZA A NAVEGAR:
...
// -----------------------------------------------------------------------------------------------------------------------
		...
		do {
			...
			if (msgAlerta != null) {
				...
				if (Util.containsIgnoreCaseAndAccents(msgAlerta, "SU CLAVE VIRTUAL EMPRESARIAL HA EXPIRADO"))
					msgError = "\nRecuerde cambiar la data con la nueva clave y si tiene otros clientes, revise sus claves antes de reiniciar la AutomatizaciÃ³n.";
...
// -----------------------------------------------------------------------------------------------------------------------
			...
			if (msgAlerta != null) {
				...
				if (msgAlerta.contains("ya existe una sesiÃ³n abierta"))
				...
				intento++;
				Reporter.reportEvent(Reporter.MIC_INFO, "*** Ya existe una sesiÃ³n abierta");
...
// =======================================================================================================================
	 ...
	 * mensaje de error.
	 * 
	 * @param condicion - Condiciï¿½n del logueo a realizar.
	 */
	private String loginFrontFirmas(String condicion) throws Exception {
		...
		String msgAlerta = null;
		int intento = 0; // PARA SABER CUANTOS INTENTOS PARA HACER EL INGRESO, Sï¿½LO SE PERMITIRA 2
							// INTENTOS
		...
		do {
			...
			if (msgAlerta != null) {
				...
				if (msgAlerta.contains("ya existe una sesiÃ³n abierta"))
					msgAlerta = "1"; // PARA QUE INTENTE DE NUEVO
				intento++;
				Reporter.reportEvent(Reporter.MIC_FAIL, "ya existe una sesiÃ³n abierta");
			}
		} while (msgAlerta != null && intento < NUM_MAX_INTENTO_LOGIN); // CUANDO NO HAY ALERTA SE PUDO INGRESAR AL
																		// PORTAL
		// -----------------------------------------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------------------------------------        
		...
		// Sï¿½LO EN EL FRONT : INGRESA EL Nï¿½MERO DE CLIENTE EMPRESARIAL
		if (isFront) {
...
//-----------------------------------------------------------------------------------------------------------------------        
		// INGRESA EL Nï¿½MERO DE DOCUMENTO DE IDENTIFICACIï¿½N
		this.write(this.element(cmNumDocu + cmTexto), numDoc);
//-----------------------------------------------------------------------------------------------------------------------        
		// Sï¿½LO EN FRONT Y SI NO SE HA DESPLEGADO EL CAMPO DE LA CLAVE : DA CLICK EN
		// INGRESAR
		String msgAlerta = null;
...
//-----------------------------------------------------------------------------------------------------------------------        
		boolean isFront = (this.portalCargado.equals(DatosEmpresarial.PORTAL_FRONT_PYME));
		// Sï¿½LO EN EL FRONT : INGRESA EL NÃšMERO DE CLIENTE EMPRESARIAL
		if (isFront) {
			this.getJse().executeScript(scriptReloadPage);
			Util.wait(1);
			do {
...
//-----------------------------------------------------------------------------------------------------------------------        
			// SELECCIONA EL TIPO DE IDENTIFICACIÃ“N
			String msgError = this.selectListItem(this.element(cmTipoId + cmSelect), tipoDoc);
			if (msgError!=null) {
...
// ***********************************************************************************************************************
	...
	public void SiExistVerificacionRobot() {
		...
		if (this.element(By.id("main-iframe")) != null) {
			...
			WebElement iframe = this.element(By.id("main-iframe"));
			this.getDriver().switchTo().frame(iframe);
			if (this.element(accesDenegado) != null) {
				Util.wait(5);
				WebElement iframe2 = this.element(By.xpath("/html/body/div[1]/div[2]/div[2]/div/div/div/div/div/iframe"));
				...
				if (this.element(imNOtaRobot) != null) {
					...
					this.getDriver().switchTo().frame(iframe);
					Util.wait(6);
					WebElement iframe3 = this.element(By.xpath("/html/body/div[3]/div[4]/iframe"));

...
//===================================================================================================================================
	...
	public String seleccionarCuenta(String servicio, String tipoIdEm, String nitEmpre, String tipoCuenta,
		...
		do {
			...
			radiobutton = this.element(this.locRadiobutton.replace("NUMCUENTA", Util.right(numeroCuenta, 4)).replace("TIPOCUENTA", tipoProd));
		...
		if (radiobutton == null) {
			Evidence.save("No se encuentra el registro " + tipoProd + " - " + numeroCuenta,this);
...
//				this.validacionSaldosStratus(servicio, tipoIdEm, nitEmpre, tipoProd, numeroCuenta, true);
		...
		Evidence.save("Cuenta seleccionada, ventana origen",this);
	...
	public String ingresarValores(String tipoMoneda, String monto, String concepto, String numeral1, String numeral2,
		...
		try {
			...
			if (this.element(inputMoneda).findElements(option).size() > 0) {
				...
				if (isValid(msg))
				if (msg.equals("Valor no en contrado")|| msg.contains("Valor ya seleccionado")) {
			...
			this.getDriver().switchTo().defaultContent();
			Evidence.save("No aparecen opciones en la lista desplegable",this);
...
//		this.element(inputMonto).sendKeys(monto);
		...
		try {
			...
			if (this.element(inputConcepto).findElements(option).size() > 0) {
				...
				if (isValid(msg))
				if ( msg.equals("Valor no en contrado")) {
		...
		if (concepto.contains("1 -") || concepto.contains("3 -")) {
			...
			do {
				...
				if (contador >= 30) {
					...
					this.getDriver().switchTo().defaultContent();
					Evidence.save("No carga numeral1 completo",this);
...
//				this.element(inputDescripcion).sendKeys(descripcion);
		...
		if (!concepto.contains("4 -")) {
			...
			if (!valorNumeral1.equals("")) {
				...
				} while (btnAgregarNumeralCambiario == null && salir == false);
				...
				} while (inputNumeral == null && salir == false);
...
//					this.element(inputValorNumeral1).sendKeys(valorNumeral1);
					...
					this.getDriver().switchTo().defaultContent();
					Evidence.save("No aparecen el campo valo",this);
...
//					this.element(inputValorNumeral2).sendKeys(valorNumeral2);
					...
					this.getDriver().switchTo().defaultContent();
					Evidence.save("No aparecen el campo valor",this);
					return "No aparecen el campo valo";
				}
			} else {
		...
		Evidence.save("Valores ingresados en ventana 'Valores'",this);
		...
		if (this.element(popupConfirmacion) != null) {
			...
			Evidence.save("popUp en valores",this);
...
//		}
		...
		Evidence.save("Datos - Beneficiario",this);
...
//		this.element(inputReferenciaPago).sendKeys(referenciaPago);
		...
		Evidence.save("InfoBeneficiario",this);
		...
		try {
			...
			Evidence.save("Paso correcto a ventana Destino 2da parte",this);
		}
...
//===================================================================================================================================
	...
	public String SeleccionarBeneficiario(String nombreBene, String paisDestino, String cuentaBene, String FechaBene)
		...
		if (objRadioButton == null) {
			...
			Evidence.save("Error en ventana 'Destino 1ra parte'",this);
		...
		try {
			...
			Evidence.save("Paso correcto a ventana Destino",this);
		}
...
//===================================================================================================================================
	...
	public String ValidarBeneficiario(String nombreBene, String paisDestino, String ciudadDestino, String direccionBene,
		...
		do {
			...
			if (contador >= 30) {
				this.getDriver().switchTo().defaultContent();
				Evidence.save("No se encuentra el campo : Nómbre del beneficiario",this);
				return "TimeOut no se encontro el campo : Nómbre del beneficiario";
			}
		...
		do {
			...
			if (contador >= 30) {
				this.getDriver().switchTo().defaultContent();
				Evidence.save("No se encuentra el campo : País destino",this);
...
//		this.element(inputNumeroCodigo).sendKeys(numeroCodigo);
		...
		try {
			...
			Evidence.save("No se encuentra el banco beneficiario",this);
			...
			do {
				...
				if (contador >= 30) {
					this.getDriver().switchTo().defaultContent();
					Evidence.save("No se encuentro el boton_siguiente",this);
...
//			this.element(inputNumeroCodigoInter).sendKeys(numeroCodigoInter);
			...
			do {
				...
				contador++;
				if (contador == 4) {
					Evidence.save("No se encuentra el banco beneficiario intermediario",this);
		...
		do {
			...
			switchToFrameDivisas();
			if (contador >= 30) {
				Evidence.save("No se encuentro el boton siguiente",this);
				this.getDriver().switchTo().defaultContent();
				return "No se encuentro el boton_siguiente";
			}
		...
		try {
			...
			Evidence.save("Paso correcto a ventana 'Cotizacion'",this);
		}
	...
	public String ValidaciondatosSelecDestino(String tipoCodigo, String numeroCodigo, String intermediario,
		...
		if (intermediario.equals("SI")) {
			...
			do {
				Util.wait(1);
				if (contador >= 30) {
					Evidence.save("No se encuentra la opción  intermediario",this);
					this.getDriver().switchTo().defaultContent();
			...
			do {
				Util.wait(1);
				if (contador >= 30) {
					Evidence.save("No se encuentra el Tipo codigo intermediario",this);
					this.getDriver().switchTo().defaultContent();
...
//			this.element(inputNumeroCodigoInter).sendKeys(numeroCodigoInter);
			...
			do {
				...
				contador++;
				if (contador >= 30) {
					Evidence.save("No se encuentra el banco beneficiariointermediario",this);
					this.getDriver().switchTo().defaultContent();
	...
	public String confirmacion(String intermediario) throws Exception {
		...
		do {
			...
			if (contador >= 30) {
				...
				Evidence.save("No se encuentra el modulo: Confirmar",this);
				this.getDriver().switchTo().defaultContent();
	...
	public void validacionSaldosStratus(String servicio, String tipoDocumento, String numeroDoc, String tipoCta,
		...
		if (DatosDavivienda.STRATUS != null) {
			...
			if (saldoOrigen) {
				...
				saldoDis = this.getText(this.element(locSaldodisponible.replace("NUMCUENTA", Util.right(numCta, 4))
